...
def foodHeuristic(state, problem):
    ...
    for a in range(0, len(foodToVisit)):
        for food in foodToVisit:
            distances[food] = distance.manhattan(position, foodList[foodToVisit[food]])

        min = -1
        for food in foodToVisit:
            if min == -1 or distances[min] > distances[food]:
                min = food

        foodToVisit.remove(min)
        counter += distances[min]
